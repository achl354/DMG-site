...
/**
 * Node index each of the 6 equally-spaced icon-glow time slots belongs to
 * -- [0, 5, 4, 3, 2, 1], not [0, 1, 2, 3, 4, 5], so each node's icon glow
 * (see .nodeIconPulse's own comment) steps anticlockwise around NODES'
 * own clockwise layout (see that constant's comment) instead of matching
 * it, independently of the traveling pulse's own orbit direction below --
 * confirmed deliberately desynced from it (the pulse stays clockwise,
 * matching NODES' own layout, while just this glow sequence reverses) per
 * explicit feedback, not an oversight. This exact array is a self-inverse
...
/**
 * Closed hexagon through 6 points at ORBIT_RADIUS_PCT, in NODES' own
 * (clockwise) order -- not rendered as a visible ring, just the traveling
 * pulse's motion path (see the `idle` block below). Computed once at
 * module scope, not per render -- it only depends on the constant above
 * and the fixed NODES list, never on `progress`/`idleDrift`, so
 * recomputing it on every scroll-driven re-render bought nothing.
 */
const orbitPathD =
                ...
                style={
                  {
                    ...
                    // across the same 7s period pulseOrbit uses, but in
                    // NODE_VISIT_ORDER's anticlockwise order rather than
                    // each node's own index -- see that constant's comment:
                    // this deliberately no longer lines up with the
                    // traveling pulse's own (still clockwise) orbit, just
                    // gives the glow sequence itself an anticlockwise feel.
